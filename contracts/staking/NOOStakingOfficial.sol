...
import "../DGP/NOOClient.sol";

contract NOOStakingOfficial is ICharacterUpdatedHandler, NOOClient, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using MGPLibV2 for uint256;
    using MGPLibV2 for EnumerableSet.AddressSet;

    event PoolUpdated(uint256 pId, uint256 minRarity, uint256 minRank, uint256 agPerBlock, uint256 dgtPerBlock);
    event WithdrawPendingToken(address indexed account, uint256 pId, uint256 globalTokenId, uint256 ag, uint256 dgt);
    ...
    EnumerableSet.AddressSet private _tokenUpdaters;
    ...
    function addTokenUpdater(address tokenUpdater_) external onlyOwner {
        require(_tokenUpdaters.add(tokenUpdater_), "already a tokenUpdater");
    }

    function removeTokenUpdater() external view returns (address[] memory) {
        return _tokenUpdaters.toArray();
    }

    function tokenUpdaters(address tokenUpdater_) external onlyOwner {
        require(_tokenUpdaters.remove(tokenUpdater_), "already a tokenUpdater");
    }
     ...
     */
    function deposit(uint256 pId, bytes calldata compositeData) external {
        uint256[] memory globalTokenIds = _validOwnerAndEmptyPoolId(compositeData);
        _updatePool(pId);
    ...
    function _validOwnerAndEmptyPoolId(bytes calldata compositeData) private returns (uint256[] memory globalTokenIds) {
        address account = msg.sender;
        globalTokenIds = _validateOwnerBatch(account, compositeData);
        require(globalTokenIds.length > 0, "no token");
    ...
    function _validOwnerAndGetPoolId(bytes calldata compositeData) private returns (uint256[] memory globalTokenIds, uint256 pId) {
        address account = msg.sender;
        globalTokenIds = _validateOwnerBatch(account, compositeData);
        require(globalTokenIds.length > 0, "no token");
     ...
     */
    function withdraw(bytes calldata compositeData) external {
        (uint256[] memory globalTokenIds, uint256 pId) = _validOwnerAndGetPoolId(compositeData);
     ...
     */
    function withdrawPendingTokenBatch(bytes calldata compositeData) external {
        (uint256[] memory globalTokenIds, uint256 pId) = _validOwnerAndGetPoolId(compositeData);
        _withdrawPendingTokenBatch(pId, msg.sender, globalTokenIds);
    }
    ...
    function _withdrawPendingTokenBatch(
        ...
        for (uint256 index = 0; index < globalTokenIds.length; ++index) {
            ...
            emit WithdrawPendingToken(account, pId, globalTokenId, agPending, dgtPending);
        }
    }
     ...
     */
    function pendingAG(uint256 globalTokenId) external view returns (uint256 pending) {
     ...
     */
    function pendingDGT(uint256 globalTokenId) external view returns (uint256 pending) {
        (address contractAddress, uint256 poolId) = _tokenTracker.getTrace(globalTokenId);
